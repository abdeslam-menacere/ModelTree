...
`.github/workflows/skills-ci.yml` and `web/src/data/**`, so it reports no check
at all on a pull request that touches none of them — a documentation-only or
`tools/updater/`-only change, for instance — and each of those would sit pending
