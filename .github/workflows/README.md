...
| [`instruction-references.yml`](instruction-references.yml) | `pull_request` and `push` to `main`, path-filtered to `.github/copilot-instructions.md`, `.github/skills/**`, `tools/instruction_refs/**` and `.github/workflows/instruction-references.yml`, `workflow_dispatch` | Resolves the paths, issue citations, and section markers in the instructions file, and every issue citation in the skill documents. Not quite every path: a backticked reference the file wraps across two lines stays in phase, so the reference after it is still checked, but the wrapped one is not itself resolved — what the document renders is its two fragments joined by a space, which is not a path, and joining them without the space would be a guess at what the author meant |
