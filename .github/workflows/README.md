...
| [`skills-ci.yml`](skills-ci.yml) | `pull_request` (every one), `merge_group` (every queue entry) and `push` to `main`, `workflow_dispatch`; scoped **inside the job** to `.github/skills/**`, `.github/scripts/**`, `.github/agents/**`, `.github/copilot-instructions.md`, `.github/workflows/skills-ci.yml` and `web/src/data/**`, with every event other than a pull request running the gates outright | The data-refresh gates' self-tests, `gate-dataset` run against the live dataset, `gate-reversals` run over `refresh-runs.json` and the dataset together — refusing a record the automated panel rejected that is in `web/src/data/` today with no entry in `rejection-reversals.json` saying which change brought it back and on what evidence (#835); it gates *visibility*, so an ordinary reviewed change may overrule the panel and an objection may be recorded as still unanswered, but neither may happen silently — and a refusal of a hand-written test count in the skill documentation — a numeral described as tests, self-tests, test cases or assertions, in either order, with markdown emphasis tolerated around the numeral, and a table column whose heading is one of those nouns and whose body cell is a bare number. Noun-first needs a real separator, of the kind a label or a table cell supplies (`tests: 103`), so the verb reading — "the gate tests 4 kinds of emptiness" — is not a count, and nor is a year after `in` or `since`, a written-out number, a singular `N test`, or `N checks`, which in this repository usually means a status check. It reads one line at a time, so a count split across two lines of prose is not seen, and where it errs it over-matches: "adds 3 tests" is flagged although it sizes a change rather than the suite. The script header carries the same list with the reasoning. Since [#652](https://github.com/abdeslam-menacere/ModelTree/issues/652) it also refuses a fenced `bash`, `sh` or `powershell` block in `.github/copilot-instructions.md`, `.github/agents/**` or `.github/skills/**` that Windows PowerShell 5.1 cannot **parse** — `&&` and `\|\|`, a `\` line continuation whose next line opens a `--flag`, an unescaped `<` used as a redirection or heredoc, and an unterminated string — after substituting `<placeholder>` tokens, which are documentation rather than defects and which a check that read them raw would report on every documented command. A block that quotes another tool's input rather than prescribing something to run is exempted by a `<!-- shell-parse-exempt: reason -->` marker whose reason may not be empty, and every exemption is named on every run including a passing one. The scope and parse-location decisions behind it are recorded under **The two decisions behind `check-shell-invocations.mjs`** below. Since [#1040](https://github.com/abdeslam-menacere/ModelTree/issues/1040) a further step runs `check-shell-invocations.test.mjs`, which pins that script's *fence model* — both markers, closure only on the same marker at a run at least as long, indentation, and the exemption lookback — by running the committed script over fixture corpora in a temporary repository root, each case carrying a stated mutation it must go red under. It is a step of this job rather than a vitest file because this job's scope covers `.github/scripts/` as a directory while `web-ci`'s names `ci-preflight.mjs` file by file, so a vitest copy would never be selected by an edit to the script it pins. Since [#863](https://github.com/abdeslam-menacere/ModelTree/issues/863) it also refuses a gate definition under `.github/agents/` that instructs a gate to read the party it is judging. `.github/copilot-instructions.md` says a gate judges from the issue text and `git diff` only; both gate definitions told a gate to take its operating policy, and QA its acceptance criteria, from `DOCK.md`, which the developer under review writes and can edit. Each covered definition must now carry a 32-line permitted-inputs contract **verbatim** between `<!-- gate-inputs:begin -->` and `<!-- gate-inputs:end -->`, byte-compared against the canonical copy in the checker, and nothing outside those blocks may name one of five enumerated routes to the developer's own account — the worktree file, a `--comments` flag, a `--json` projection selecting comments, the comments REST endpoint, or a GraphQL timeline query. The primary assertion is positive rather than a bad-string sweep precisely so that there is no exemption marker to widen: the only excluded region is the contract, and every byte of it is pinned. It measures repository content and not runtime behaviour, and says so on every run, because no gate transcript is repository content and a check that claimed to police what an agent read would be reporting on something it cannot see. The decisions behind it are recorded under **The three decisions behind `check-gate-independence.mjs`** below. Since [#1086](https://github.com/abdeslam-menacere/ModelTree/issues/1086) three further steps close the gap that let a test file exist without ever running: one runs `check-test-registries.mjs`, which refuses a `*.test.mjs` under `.github/` that no registry names and a registration naming a file that does not exist; one runs `check-test-registries.test.mjs`, which pins that checker over fixture corpora in a temporary repository root; and one runs `check-bundle-pairing.test.mjs`, the scout skill's suite, which was the orphan that prompted the issue — a whole suite of passing tests that had never run in CI. The decisions behind the checker are recorded under **The four decisions behind `check-test-registries.mjs`** below |
...
| [`source-link-health.yml`](source-link-health.yml) | `schedule` (weekly), `pull_request` (every one), `workflow_dispatch`; the hermetic job scoped **inside the job** to `web/src/data/sources.json`, `web/src/data/releases.json`, `.github/scripts/source-link-health/**` and `.github/workflows/source-link-health.yml`, and the networked job to `web/src/data/sources.json` alone | Requests every recorded primary source URL, and on a schedule or dispatch every `license.url` a release records as well, and reports the ones that are definitively broken (404, 410) or permanently moved (301, 308), grouped by URL so one dead link is one finding however many records cite it. Each finding names its provenance, so a release is never listed as a source record. A rate-limit, a block, a timeout and a 5xx are each classified and **never** reported as a finding, because none of them is evidence the link is bad. On a schedule it opens or updates one maintenance issue and closes it when a later sweep is clean; on a pull request the networked sweep job can only report, and only about source URLs that pull request itself introduced. Whether a licence URL *stays* reachable is upstream decay, which a diff-scoped run cannot see, so it remains on the clock ([ADR 0017](../../docs/adr/0017-licence-urls-are-swept-on-the-clock-not-on-the-diff.md)). Whether a licence URL *ever* resolved is a different question, and since [ADR 0019](../../docs/adr/0019-introduction-time-licence-urls-are-checked-on-the-diff.md) the separate `licence-link-introduction` job asks it on the diff, on pull requests only, over the licence URLs that pull request introduced or re-pointed — measured at 28 net-new third-party requests across 50 commits, zero for 74% of them. The extraction dry run covers licence URLs on every run including a pull request, because it makes no request at all: a licence URL that is *malformed* fails on the diff that introduced it. Neither job ever writes to `web/src/data/` |
...
| `licence-link-introduction` | `source-link-health.yml` | **No, and never** — same reason as `source-link-health`: it requests third-party URLs ([ADR 0019](../../docs/adr/0019-introduction-time-licence-urls-are-checked-on-the-diff.md)) |
...
| `licence-link-introduction` | same reason: it requests third-party licence URLs, so its verdict is partly somebody else's uptime ([ADR 0019](../../docs/adr/0019-introduction-time-licence-urls-are-checked-on-the-diff.md)) |
...
**What pins its fence model, and why the pin lives in this job.** A third
decision, added by
[#1040](https://github.com/abdeslam-menacere/ModelTree/issues/1040) after the
first two shipped. The script decides what a fenced block *is* before it decides
whether the block parses: `fencedBlocks` models both markers, closes a fence only
on its own marker at a run at least as long, strips the opening fence's
indentation from the body, and reads the exemption marker off the line above.
None of that was pinned. The script's own `MUST_FLAG`/`MUST_NOT_FLAG` self-test
feeds command *strings* straight to the parser and never constructs a document,
so an edit narrowing the fence regex to backticks alone would have left every
check in this job green while silently dropping every tilde-fenced block from the
scan — and a scan that reads less is a scan that finds less, which is the failure
this whole check exists to make impossible. `check-shell-invocations.test.mjs`
runs the committed script over fixture corpora in a temporary repository root, so
it exercises the shipped bytes rather than a reimplementation, and every case
carries a stated one-token mutation of the script's source that it is required to
go red under — a pin that cannot fail to fail.

It is a step of **this** job rather than a vitest file under `web/tests/` for a
reason worth stating, because the alternative looks equivalent and is not.
`web-ci`'s in-job scope names `.github/scripts/ci-preflight.mjs` file by file,
and `check-shell-invocations.mjs` is not in that list; `skills-ci`'s scope names
`.github/scripts/` as a whole directory. A vitest copy of this suite would
therefore never be selected by the very edit it exists to catch — present in the
tree, absent from the run, and reporting nothing either way. Widening `web-ci`'s
allowlist instead would pull a full site build onto every edit of a Node gate
script for no added coverage.

### The three decisions behind `check-gate-independence.mjs`

That script is a step of `skills-ci`. It refuses a gate definition under
`.github/agents/` that instructs a gate to read the party it is judging — the
contradiction
[#863](https://github.com/abdeslam-menacere/ModelTree/issues/863) is about.
`.github/copilot-instructions.md` says a gate judges from the issue text and
`git diff` only, while both gate definitions told a gate to take its operating
policy — and, for QA, its acceptance criteria — from `DOCK.md`, a file the
developer under review writes and can edit. Nothing measured the disagreement,
so it survived 222 recorded verdicts. It never contaminated one, but only
because no `DOCK.md` has ever been created in this pipeline: the protection was
an accident of how docks are provisioned rather than any rule, and one command
would have ended it.

**What it measures, in its own terms before anyone's preferred terms.** It
measures **repository content**: what the shipped gate definitions instruct. It
does **not** measure what any gate agent read at run time, and the gap is not a
hedge. No gate transcript is repository content — `.drydock/tmp/` and `.docks/`
are both gitignored, and `.drydock/docks/10.json` is a single fossil — so there
is nothing an audit could read. `.github/copilot-instructions.md` argues
explicitly against pretending otherwise, in the passage that introduces the
route property: a checker sweeping for one spelling reports green while another
route returns the same bytes, and dressing prose up as enforcement it cannot
perform is worse than leaving it as prose. Runtime therefore stays a property an
agent applies. What is versioned text can still be pinned, and that is what this
does.

**Why the primary assertion is positive, not a bad-string sweep.** Each covered
definition must carry a 32-line permitted-inputs contract **verbatim**, between
`<!-- gate-inputs:begin -->` and `<!-- gate-inputs:end -->`, byte-compared
against `CANONICAL_LINES` in the checker. A negative sweep would need an
exemption marker for the contract itself — which names `DOCK.md` and `--json
comments` in order to forbid them — and an exemption marker is a hole the size
of the thing being checked, since the next author to want the old behaviour
writes a marker rather than an argument. Here the *only* excluded region is the
contract, and every byte of it is asserted, so nothing arbitrary can hide inside
the carve-out. The canonical text lives in the checker rather than in a shared
data file both sides load: with the text held elsewhere the invariant degrades
to "the copies agree", which three coordinated edits satisfy while asserting
nothing about what they agree on. The five forbidden-route rules scanned outside
the blocks are the secondary half, and they find *instances*; the class is
carried by the contract's own property sentence, which the checker separately
requires to still be present in `.github/copilot-instructions.md` so the two
documents cannot drift apart in silence.

**Why it fails closed, and what its controls land on.** Exit 0 clean, 1 a
violation, 2 the checker could not answer — any argument, an unreadable file, an
empty covered set, or a scan that examined no lines is 2, never a pass, because
a checker that finds nothing because it looked at nothing is the exact defect
this issue is about. Before it reads a repository file it runs its own rules
against samples that must fire and controls that must stay silent, and the
samples land on the failing path rather than merely differing from the subject:
a phrase wrapped across a line break and a phrase interrupted by inline markup,
both asserted to be *missed* by a naive `includes`, so a matcher that had stopped
normalising would fail its own self-check rather than reporting a clean sweep. A
nonce generated at run time proves the negative arm can come back negative, and a
mutation of the pinned contract proves the byte comparison can come back
differing. Every run, including a passing one, prints what the check does not
cover.

### The four decisions behind `check-test-registries.mjs`

Nothing under `.github/` discovers its tests by globbing. `skills-ci.yml`,
`source-link-health.yml` and `ci-preflight.mjs` each enumerate the filenames by
hand, and none of them can see the directory they are drawn from. A `*.test.mjs`
added without an entry is therefore not a failing test and not a skipped test —
it is a file nothing ever opens, and it reports nothing at all.
`check-bundle-pairing.test.mjs` sat in that state with a whole suite of passing
tests that had never run in CI
([#1086](https://github.com/abdeslam-menacere/ModelTree/issues/1086)).

**Each registry is measured against a directory listing, never against another
registry.** The obvious check is a cross-check — assert that the workflow and the
preflight name the same files — and it is worthless here in a way that is the
whole point. When the defect was found the two registries *agreed* about the
orphaned file: both omitted it. Agreement between two enumerations says only that
they were copied from each other, so the filesystem is the reference and each
registry is measured against it.

**Coverage is a union.** A file is covered when at least one registry names it.
Requiring every registry to name every file would be wrong for
`link-health.test.mjs`, which belongs to `source-link-health.yml` and has no
business in `skills-ci.yml`, whose scope step does not select for it. The cost is
worth stating because it is what a reader will test first: removing one of two
registrations for the same file **passes**, by design, and it is the removal of
the last one that fails. The fixture corpora in `check-test-registries.test.mjs`
therefore include files named by exactly one registry, where removing that one
registration does fail, so the per-registry direction is demonstrated somewhere
even though the repository as it stands cannot demonstrate it.

**A registration is a path inside a `--test` argument region.** One rule covers
all three syntaxes in use — the YAML `run: node --test <path> <path>` lines and
both the single-line and multi-line `args: ['--test', …]` arrays — so there is no
YAML parser and no JS parser to keep in step with either. Whole-line comments are
stripped first, so the `# … node --test <directory> …` notes both workflows carry
above their real commands cannot be read as registrations. Requiring the flag is
the conservative direction: a file merely *mentioned* is not a file that runs, and
counting a mention would be a false pass, whereas a registration the rule fails to
see is reported as an orphan and read by a human.

**An empty scan is refused rather than passed.** Exit 0 clean, 1 a defect, 2 the
checker could not answer — any argument, an unreadable registry, an empty
directory listing, or a registry that yields no registration at all is 2, never a
pass. The asymmetry is the reason: a broken *extractor* fails loudly on its own,
because every file on disk would read as an orphan, but a broken *listing* fails
silently — nothing found, so nothing unregistered, so a clean report about a scan
that never happened
([ADR 0018](../../docs/adr/0018-not-looking-and-finding-nothing-are-separately-representable.md)).
Before it reads a repository file it drives both rules against synthetic corpora
that must fire, a clean corpus that must stay silent, a union corpus that must not
be read per-registry, each of the three syntaxes that must parse, and a mention
and a commented-out registration that must not count. Every run, including a
passing one, prints which registry covers which file and what the check does not
cover.

**What it does not do.** It does not require the enumerations to become globs —
that alters which tests run in CI and is left to its own issue; the guard holds
either way. It does not look outside `.github/`, because `web/` discovers its
tests through vitest by pattern and the defect class does not arise there. It does
not check that a registered file defines any tests. And it does not check that the
registry naming a file is one that will be *selected* for a given pull request:
a pull request confined to `.github/workflows/source-link-health.yml` does not
match `skills-ci`'s in-job scope, so the checker would not run on it, and the
`push` to `main` — which runs this job unfiltered — is what catches that case.
...
- **`licence-link-introduction`.** Same shape, different question: it requests
  the licence URLs a pull request introduces
  ([ADR 0019](../../docs/adr/0019-introduction-time-licence-urls-are-checked-on-the-diff.md)).
  Advisory, never required, network-bound.
